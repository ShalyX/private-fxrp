...
#   EXISTING_INSTRUCTION_SENDER — reuse an already deployed sender (optional)
#
# Outputs:
...
step 2 "Resolve InstructionSender contract"
cd "$PROJECT_DIR/tools"
: > "$LOG_FILE"  # truncate log file
if [[ -n "${EXISTING_INSTRUCTION_SENDER:-}" ]]; then
    INSTRUCTION_SENDER="$EXISTING_INSTRUCTION_SENDER"
    log "Using existing InstructionSender: $INSTRUCTION_SENDER"
else
