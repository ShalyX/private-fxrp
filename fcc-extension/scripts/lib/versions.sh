...
#   TEE_PROXY_COMMIT immutable tee-proxy commit used by proxy/Dockerfile
#
...
load_versions() {
    ...
    TEE_PROXY_COMMIT="$(grep -E '^ARG TEE_PROXY_COMMIT=' "$root/proxy/Dockerfile" 2>/dev/null | head -1 | cut -d= -f2)"

    export TEE_NODE_VERSION TEE_NODE_REF TEE_PROXY_COMMIT
}
