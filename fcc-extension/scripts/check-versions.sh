...
# --- 3. tee-proxy: module pseudo-version must match Dockerfile's commit ---
TOOLS_PROXY="$(pin "$TOOLS_GOMOD" github.com/flare-foundation/tee-proxy)"
TOOLS_PROXY_REF="$(_versions_ref "$TOOLS_PROXY")"
if [[ -z "$TEE_PROXY_COMMIT" ]]; then
    warn "no ARG TEE_PROXY_COMMIT in proxy/Dockerfile — skipping cross-check"
elif [[ -z "$TOOLS_PROXY" ]]; then
    warn "no tee-proxy pin in $TOOLS_GOMOD — skipping cross-check"
elif [[ "$TEE_PROXY_COMMIT" != "$TOOLS_PROXY_REF"* ]]; then
    echo -e "${RED}  tee-proxy mismatch:${NC}" >&2
    echo "    $TOOLS_GOMOD:        $TOOLS_PROXY" >&2
    echo "    proxy/Dockerfile:    $TEE_PROXY_COMMIT" >&2
    FAILED=1
else
    log "tee-proxy      $TOOLS_PROXY (module == pinned commit)"
fi
